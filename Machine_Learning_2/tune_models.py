...
MODELS_DIR = BASE_DIR / "outputs" / "models_tuned"
...
def tune_one_label(algo_name: str, algo_config: dict,
    ...
    # Gop Train+Val lai (RandomizedSearchCV can toan bo data + chi dinh fold qua cv=)
    X_combined = np.vstack([X_train, X_val])
    y_combined = np.concatenate([y_train, y_val])
 
    predefined_split = make_predefined_split(len(X_train), len(X_val))
